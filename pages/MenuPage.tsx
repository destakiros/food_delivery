    ...
    const fetchMenu = async () => {
      try {
        ...
        if (data && Array.isArray(data) && data.length > 0) {
          ...
          // If data is null or empty, use mock data
          setFoods(MOCK_FOODS);
        }
      } catch (error) {
        setFoods(MOCK_FOODS);
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-12 mb-12">
            <div>
              <h1 className="text-5xl lg:text-7xl font-black text-gray-900 tracking-tighter mb-4 uppercase">
                The <span className="text-orange-600">Daily</span> Menu
              </h1>
              <p className="text-xl text-gray-500 font-bold uppercase tracking-widest text-xs">Quality You Can Taste.</p>
            </div>
            <div className="relative group w-full md:max-w-md">
              <i className="ph ph-magnifying-glass absolute left-6 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-orange-500 transition-colors text-xl"></i>
              <input 
                type="text" 
                placeholder="Find a Burger or Shake..." 
                className="w-full pl-14 pr-6 py-5 bg-white rounded-3xl border-none shadow-xl shadow-gray-200/50 ring-1 ring-gray-100 focus:ring-4 focus:ring-orange-500/10 transition-all outline-none text-lg font-bold"
                value={searchTerm}
        ...
        {filteredFoods.length > 0 ? (
          ...
          <div className="text-center py-40 bg-white rounded-[4rem] shadow-xl border border-gray-100">
            ...
            <button 
              ...
              className="px-12 py-5 bg-orange-600 text-white rounded-[2rem] font-black text-xl hover:bg-orange-700 shadow-2xl transition-all"
            >
              Show All Items
            </button>
          </div>
